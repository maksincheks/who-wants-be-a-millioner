...
"    color: rgb(255, 255, 255);")
        ...
        self.questions = {
            ...
            "Какое насекомое вызвало короткое замыкание в ранней версии\n вычислительной машины, "
            "тем самым породив термин «компьютерный баг»?": ["Мотылек", "Таракан", "Муха", "Японский хрущик"],
            "Из каких плодов можно получить копру?": ["Кокос", "Вишня", "Абрикос", "Арбуз"],
            "Какой химический элемент составляет более"
            " половины массы тела человека?": ["Кислород", "Кальций", "Кислород", "Железо"],
            "Какую часть тела также называют «атлант»?": ["Шейный позвонок", "Шестая пара ребер", "Головной мозг", "Часть плеча"],
            "Сколько кубиков в кубике Рубика?": ["26", "24", "22", "28"],
            "Кто первый полетел в космос?": ["Гагарин", "Терешкова", "Масленников", "Пермяков"],
            "Что изображено на заднем плане картины Леонардо да Винчи\n «Мона Лиза»?": ["Пейзаж", "Стадо овец", "Город", "Драпировка"],
            "Что означает гавайское слово «вики»,\n которое дало название"
            " интернет-энциклопедии «Википедия»?": ["Быстрый", "Маленький", "Простой", "Умный"],
            "Какой специалист занимается изучением \n"
            "неопознанных летающих объектов? ": ["Уфолог", "Кинолог", "Психиатр", "Звездочёт"],
    ...
    def check_answer(self, answer):
        if answer == self.correct_answer:
            ...
            if self.correct_answers_count == 15:  # Проверяем, достиг ли игрок 15 правильных ответов
                reply = QMessageBox()
                reply.setWindowTitle("Победа")
                reply.setText("Вы выиграли!!!")
            else:
                self.show_question()  # Показываем следующий вопрос

        else:
            reply = QMessageBox()
            reply.setWindowTitle("Проигрыш")
            reply.setText("Вернуться в главное меню? Правильный ответ: {}".format(self.correct_answer))
            reply.setStandardButtons(QMessageBox.Yes)
            reply.button(QMessageBox.Yes).setText('Да')

            # Устанавливаем стиль для кнопок
            reply.setStyleSheet("QPushButton { width: 100px; margin-left: auto; margin-right: auto; }")

            reply.setWindowFlag(QtCore.Qt.FramelessWindowHint)
