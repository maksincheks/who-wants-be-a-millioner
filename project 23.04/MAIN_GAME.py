...
"    color: rgb(255, 255, 255);")
    ...
    def show_question(self):
        ...
        for option in options:
            option.show()
        question = list(self.questions.keys())[0]
        options = self.questions.pop(question)
    ...
    def open_set(self):
        ...
        self.saved_hin3_used = self.hint3_used
    ...
    def clicked_hint1(self):
        if not self.hint1_used:  # Проверяем, использовалась ли уже подсказка
            ...
            self.volume_timer.start(2000)
            self.hint1_used = True
    def restore_volume(self):
        self.media_player.setVolume(10)
    def clicked_hint2(self):
        if not self.hint2_used:  # Проверяем, использовалась ли уже подсказка
            self.hint_2.setStyleSheet("QPushButton {\n"
                                      "    color: red;\n"
                                      "    border-radius: 20px; \n"
                                      "    background-color: rgb(170, 0, 255);\n"
    ...
    def clicked_hint3(self):
        if not self.hint3_used:  # Проверяем, использовалась ли уже подсказка
            self.hint_3.setStyleSheet("QPushButton {\n"
                                      "    color: red;\n"
                                      "    border-radius: 20px; \n"
                                      "    background-color: rgb(170, 0, 255);\n"
